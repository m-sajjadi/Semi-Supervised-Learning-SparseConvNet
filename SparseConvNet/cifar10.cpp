...
std::string Name = "weights/cifar10";  
...
Picture *OpenCVPicture::distort(RNG &rng, batchType type) {
  ...
  /*if (epoch <= 100 and type == TRAINBATCH) {*/
    // 2x2 identity matrix:
...
int main() {  
  ...
  ROFMPSparseConvNet cnn(
      2, 12, 96 /* 96n units in the n-th hidden layer*/, powf(2, 0.3333),
  ...
  for (epoch++; epoch <= 410; epoch++) { 
    ...
    cnn.processDataset(trainSet, index_train, batchSize, 0.003 * exp(-0.01 * epoch), 0.99);
    if (epoch % 20 == 0)
